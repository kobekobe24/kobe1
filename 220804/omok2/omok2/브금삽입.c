...
void Game_Start() {
	...
	do {                                            // do while문. 일단 한번 수행하고 조건에 맞으면 계속 루프
		...
		if (Winner_Checking(Omok_Board)) {
			PlaySound(TEXT("crazy.wav"), 0, SND_FILENAME | SND_ASYNC | SND_LOOP);// 승자를 체크하는 함수 호출 
			return;
		}
		Gotoxy(x, y);
...
int Check_4X4(int x, int y)
{
	...
	// 가로체크2
	if ((Omok_Board[y][x / 2 - 4] == 0 && Omok_Board[y][x / 2 - 3] == 2 && Omok_Board[y][x / 2 - 2] == 2 && Omok_Board[y][x / 2 - 1] == 2 && Omok_Board[y][x / 2 + 1] != 2) ||
		...
		count++;
	// 세로체크2 
	if ((Omok_Board[y - 4][x / 2] != 2 && Omok_Board[y - 3][x / 2] == 2 && Omok_Board[y - 2][x / 2] == 2 && Omok_Board[y - 1][x / 2] == 2 && Omok_Board[y + 1][x / 2] != 2) ||
		...
		count++;
	// 대각선2
	if ((Omok_Board[y - 4][x / 2 - 4] == 0 && Omok_Board[y - 3][x / 2 - 3] == 2 && Omok_Board[y - 2][x / 2 - 2] == 2 && Omok_Board[y - 1][x / 2 - 1] == 2 && Omok_Board[y + 1][x / 2 + 1] != 2) ||
